...
export default function ComplaintPortal() {
  ...
  function handleEnterComplaintClick() {
    formRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  if (!aadhaarDigits) {
    return <AadhaarLogin onSuccess={handleLoginSuccess} />;
  }
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #eff6ff 0%, #f8fafc 100%)' }}>
      <header
        style={{
          ...
          padding: '0 24px',
          background: '#ffffff',
          borderBottom: '1px solid var(--color-citizen-border)',
      ...
      >
        <Link to="/" style={{ display: 'block' }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#0f172a' }}>RoadWatch</div>
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: 0.5, color: '#64748b' }}>
        ...
        >
          <span
            style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--color-citizen-accent)' }}
          />
      ...
      {!submitted && (
        ...
        >
          ...
          <p style={{ fontSize: 16, lineHeight: 1.75, color: '#475569', margin: '0 0 32px' }}>
            Spot a pothole, waterlogged stretch, missing sign, or damaged crossing? Complaints
            submitted here go directly to the municipal RoadWatch dashboard, alongside detections
            from the city bus fleet, so repair teams can act on them.
          </p>
          <button
            onClick={handleEnterComplaintClick}
            style={{
              padding: '15px 34px',
              fontSize: 15,
              fontWeight: 600,
      ...
      <div ref={formRef} style={{ padding: submitted ? '80px 20px' : '0 20px 80px' }}>
        {submitted ? (
          ...
          >
            ...
            <p style={{ fontSize: 14, lineHeight: 1.6, color: '#475569', margin: '0 0 28px' }}>
          ...
          >
            <ComplaintForm aadhaarDigits={aadhaarDigits} onSubmitted={() => setSubmitted(true)} />
          </div>
        )}
      </div>
    </div>
